...
void SPIFFSEditor::handleRequest(AsyncWebServerRequest *request){
  ...
  if(request->method() == HTTP_GET){
...
#ifdef EDFS 
         AsyncWebServerResponse *response = request->beginResponse(_fs, "/edit_gz", "text/html", false);
#else
    ...
    if(request->hasParam("data", true, true) && _fs.exists(request->getParam("data", true, true)->value()))
	  ...
	  while (request->hasParam(String("raw") + String(k), true)) { //raw0 .. raw1
		if(f){
			i += f.print(request->getParam(String("raw") + String(k), true)->value());  
		}
		k++;
	  }
	  f.close();
	  request->send(200, "", String("IPADWRITE: ") + rawnam + ":" + String(i));
